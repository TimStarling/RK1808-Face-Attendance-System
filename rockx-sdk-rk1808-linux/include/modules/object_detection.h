...
/**
 * @brief Object Detection Result(include Face, CarPlate, Head, Object, etc...)识别结果
 */
typedef struct rockx_object_t {
    int id;                 ///< 跟踪ID
    int cls_idx;            ///< 类别索引
    rockx_rect_t box;       ///< 目标区域
    float score;            ///< 目标置信度
} rockx_object_t;

/**
 * @brief Object Array Result 数组结果
 */
typedef struct rockx_object_array_t {
    int count;                  ///< 数组数量(0 <= count < 128)
    rockx_object_t object[128]; ///< 目标对象数组
